...
function WhoUpdate()
    ...
    local whoCount = 0
    table.foreachi(MangAdmin.db.account.buffer.who, function()
        whoCount = whoCount + 1
    end)
    if whoCount > 0 then
        ...
        for line = 1, 12 do
            ...
            if lineplusoffset <= whoCount then
                ...
                if object then
                    getglobal("ma_whoscrollframe" .. line):SetText(
                        "Acct: |cffffffff" .. object["tAcc"] .. "|r Char: |cffffffff" .. object["tChar"] ..
                            "|r GMLvl: |cffffffff" .. object["tGMLevel"] .. "|r Exp: |cffffffff" .. object["tExp"] ..
                            "|r")
                    ...
                    getglobal("ma_whoscrollframe" .. line):SetScript("OnEnter", function() --[[Do nothing]]
                    end)
                    getglobal("ma_whoscrollframe" .. line):SetScript("OnLeave", function() --[[Do nothing]]
                    end)
                    getglobal("ma_whoscrollframe" .. line):SetScript("OnClick", function()
                        WhoDetail(object["tAcc"], object["tChar"], object["tMap"], object["tZone"])
                    end)
