...
MangAdmin = AceLibrary("AceAddon-2.0"):new("AceConsole-2.0", "AceDB-2.0", "AceHook-2.1", "FuBarPlugin-2.0",
    "AceDebug-2.0", "AceEvent-2.0")
...
Locale:RegisterTranslations("zhTW", function()
    return Return_zhTW()
end)
-- Register String Traslations
Strings:EnableDynamicLocales(true)
Strings:RegisterTranslations("enUS", function()
    return ReturnStrings_enUS()
end)
Strings:RegisterTranslations("zhTW", function()
    return ReturnStrings_enUS()
end)
-- Locale:Debug()
...
function MangAdmin:OnInitialize()
    ...
    if not self.db.account.style.showminimenu then
        FrameLib:HandleGroup("minimenu", function(frame)
            frame:Hide()
        end)
    end
end
...
function MangAdmin:OnClick()
    ...
    elseif (not ma_bgframe:IsVisible()) then
        ...
        FrameLib:HandleGroup("popup", function(frame)
            frame:Hide()
        end)
    end
end
...
function MangAdmin:OnTooltipUpdate()
    ...
    local ticketCount = 0
    table.foreachi(tickets, function()
    ...
    if ticketCount == 0 then
        ...
        local cat = Tablet:AddCategory("columns", 1, "justify", "LEFT", "hideBlankLine", true, "showWithoutChildren",
            false, "child_textR", 1, "child_textG", 1, "child_textB", 1)
        cat:AddLine("text", string.format(Locale["ma_TicketsNewNumber"], ticketCount), "func", function()
            MangAdmin:ShowTicketTab()
        end)
        local counter = 0
        local name
        for i, name in pairs(tickets) do
            ...
            if counter == ticketCount then
                cat:AddLine("text", string.format(Locale["ma_TicketsGoLast"], name), "func", function(name)
                    MangAdmin:TelePlayer("gochar", name)
                end, "arg1", name)
                cat:AddLine("text", string.format(Locale["ma_TicketsGetLast"], name), "func", function(name)
                    MangAdmin:TelePlayer("getchar", name)
                end, "arg1", name)
            end
        end
...
function MangAdmin:ToggleTabButton(group)
    ...
    FrameLib:HandleGroup("tabbuttons", function(button)
...
function MangAdmin:TogglePopup(value, param)
    ...
    if value == "search" then
        ...
        ma_searchbutton:SetScript("OnClick", function()
            self:SearchStart(param.type, ma_searcheditbox:GetText())
        end)
        ...
        ma_ptabbutton_1:SetScript("OnClick", function()
            MangAdmin:TogglePopup("search", {
                type = param.type
            })
        end)
        ma_ptabbutton_2:SetScript("OnClick", function()
            MangAdmin:TogglePopup("favorites", {
                type = param.type
            })
        end)
        ma_ptabbutton_2:Show()
        ma_selectallbutton:SetScript("OnClick", function()
            self:Favorites("select", param.type)
        end)
        ma_deselectallbutton:SetScript("OnClick", function()
            self:Favorites("deselect", param.type)
        end)
        ma_modfavsbutton:SetScript("OnClick", function()
            self:Favorites("add", param.type)
        end)
    ...
    elseif value == "favorites" then
        ...
        ma_modfavsbutton:SetScript("OnClick", function()
            self:Favorites("remove", param.type)
        end)
    ...
    elseif value == "mail" then
        ...
        ma_searcheditbox:SetScript("OnTextChanged", function()
            MangAdmin:UpdateMailBytesLeft()
        end)
        ma_var1editbox:SetScript("OnTextChanged", function()
            MangAdmin:UpdateMailBytesLeft()
        end)
        ma_modfavsbutton:Hide()
        ma_selectallbutton:Hide()
...
function MangAdmin:HideAllGroups()
    FrameLib:HandleGroup("main", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("char", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("npc", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("go", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("tele", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("ticket", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("server", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("misc", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("log", function(frame)
        frame:Hide()
    end)
    FrameLib:HandleGroup("who", function(frame)
        frame:Hide()
    end)
end
...
function MangAdmin:AddMessage(frame, text, r, g, b, id)
    ...
    if id == 1 then -- make sure that the message comes from the server, message id = 1
        ...
        if self.db.char.requests.toggle then
            if self.db.char.requests.item then
                ...
                for id, name in string.gmatch(text, Strings["ma_GmatchItem"]) do
                    table.insert(self.db.account.buffer.items, {
                        itId = id,
                        itName = name,
                        checked = false
                    })
            ...
            elseif self.db.char.requests.itemset then
                ...
                for id, name in string.gmatch(text, Strings["ma_GmatchItemSet"]) do
                    table.insert(self.db.account.buffer.itemsets, {
                        isId = id,
                        isName = name,
                        checked = false
                    })
            ...
            elseif self.db.char.requests.spell then
                ...
                for id, name in string.gmatch(text, Strings["ma_GmatchSpell"]) do
                    table.insert(self.db.account.buffer.spells, {
                        spId = id,
                        spName = name,
                        checked = false
                    })
            ...
            elseif self.db.char.requests.skill then
                ...
                for id, name in string.gmatch(text, Strings["ma_GmatchSkill"]) do
                    table.insert(self.db.account.buffer.skills, {
                        skId = id,
                        skName = name,
                        checked = false
                    })
            ...
            elseif self.db.char.requests.creature then
                ...
                for id, name in string.gmatch(text, Strings["ma_GmatchCreature"]) do
                    table.insert(self.db.account.buffer.creatures, {
                        crId = id,
                        crName = name,
                        checked = false
                    })
            ...
            elseif self.db.char.requests.object then
                ...
                for id, name in string.gmatch(text, Strings["ma_GmatchGameObject"]) do
                    table.insert(self.db.account.buffer.objects, {
                        objId = id,
                        objName = name,
                        checked = false
                    })
            ...
            elseif self.db.char.requests.quest then
                ...
                for id, name in string.gmatch(text, Strings["ma_GmatchQuest"]) do
                    table.insert(self.db.account.buffer.quests, {
                        qsId = id,
                        qsName = name,
                        checked = false
                    })
            ...
            elseif self.db.char.requests.tele then
                ...
                for id, name in string.gmatch(text, Strings["ma_GmatchTele"]) do
                    table.insert(self.db.account.buffer.teles, {
                        tName = name,
                        checked = false
                    })
                    PopupScrollUpdate()
        ...
        for status, char, guid, account, id, level, ip, login, latency in string.gmatch(text,
        ...
        for id, char, create, update in string.gmatch(text, Strings["ma_GmatchTickets"]) do
            table.insert(MangAdmin.db.account.buffer.tickets, {
                tNumber = id,
                tChar = char,
                tLCreate = create,
                tLUpdate = update,
        ...
        for msg in string.gmatch(text, "Ticket Message.-:.-(.*)") do
            ...
            table.insert(MangAdmin.db.account.buffer.ticketsfull, {
                tMsg = " "
            })
        ...
        for acc, char, ip, map, zone, exp, gmlevel in string.gmatch(text, Strings["ma_GmatchWho"]) do
            ...
            if acc == "Account" then
                ...
                table.insert(MangAdmin.db.account.buffer.who, {
                    tAcc = acc,
                    tChar = char,
                    tIP = ip,
                    tMap = map,
...
function MangAdmin:Favorites(value, searchtype)
    if value == "add" then
        if searchtype == "item" then
            table.foreachi(self.db.account.buffer.items, function(k, v)
                if v["checked"] then
                    table.insert(self.db.account.favorites.items, {
                        itId = v["itId"],
                        itName = v["itName"],
                        checked = false
                    })
        ...
        elseif searchtype == "itemset" then
            table.foreachi(self.db.account.buffer.itemsets, function(k, v)
                if v["checked"] then
                    table.insert(self.db.account.favorites.itemsets, {
                        isId = v["isId"],
                        isName = v["isName"],
                        checked = false
                    })
        ...
        elseif searchtype == "spell" then
            table.foreachi(self.db.account.buffer.spells, function(k, v)
                if v["checked"] then
                    table.insert(self.db.account.favorites.spells, {
                        spId = v["spId"],
                        spName = v["spName"],
                        checked = false
                    })
        ...
        elseif searchtype == "skill" then
            table.foreachi(self.db.account.buffer.skills, function(k, v)
                if v["checked"] then
                    table.insert(self.db.account.favorites.skills, {
                        skId = v["skId"],
                        skName = v["skName"],
                        checked = false
                    })
        ...
        elseif searchtype == "quest" then
            table.foreachi(self.db.account.buffer.quests, function(k, v)
                if v["checked"] then
                    table.insert(self.db.account.favorites.quests, {
                        qsId = v["qsId"],
                        qsName = v["qsName"],
                        checked = false
                    })
        ...
        elseif searchtype == "creature" then
            table.foreachi(self.db.account.buffer.creatures, function(k, v)
                if v["checked"] then
                    table.insert(self.db.account.favorites.creatures, {
                        crId = v["crId"],
                        crName = v["crName"],
                        checked = false
                    })
        ...
        elseif searchtype == "object" then
            table.foreachi(self.db.account.buffer.objects, function(k, v)
                if v["checked"] then
                    table.insert(self.db.account.favorites.objects, {
                        objId = v["objId"],
                        objName = v["objName"],
                        checked = false
                    })
        ...
        elseif searchtype == "tele" then
            table.foreachi(self.db.account.buffer.teles, function(k, v)
                if v["checked"] then
                    table.insert(self.db.account.favorites.teles, {
                        tName = v["tName"],
                        checked = false
                    })
                end
    ...
    elseif value == "remove" then
        if searchtype == "item" then
            for k, v in pairs(self.db.account.favorites.items) do
                if v["checked"] then
                    table.remove(self.db.account.favorites.items, k)
                end
            end
        elseif searchtype == "itemset" then
            for k, v in pairs(self.db.account.favorites.itemsets) do
                if v["checked"] then
                    table.remove(self.db.account.favorites.itemsets, k)
                end
            end
        elseif searchtype == "spell" then
            for k, v in pairs(self.db.account.favorites.spells) do
                if v["checked"] then
                    table.remove(self.db.account.favorites.spells, k)
                end
            end
        elseif searchtype == "skill" then
            for k, v in pairs(self.db.account.favorites.skills) do
                if v["checked"] then
                    table.remove(self.db.account.favorites.skills, k)
                end
            end
        elseif searchtype == "quest" then
            for k, v in pairs(self.db.account.favorites.quests) do
                if v["checked"] then
                    table.remove(self.db.account.favorites.quests, k)
                end
            end
        elseif searchtype == "creature" then
            for k, v in pairs(self.db.account.favorites.creatures) do
                if v["checked"] then
                    table.remove(self.db.account.favorites.creatures, k)
                end
            end
        elseif searchtype == "object" then
            for k, v in pairs(self.db.account.favorites.objects) do
                if v["checked"] then
                    table.remove(self.db.account.favorites.objects, k)
                end
            end
        elseif searchtype == "tele" then
            for k, v in pairs(self.db.account.favorites.teles) do
                if v["checked"] then
                    table.remove(self.db.account.favorites.teles, k)
                end
            end
        end
    ...
    elseif value == "select" or value == "deselect" then
        ...
        if searchtype == "item" then
            if MangAdmin.db.char.requests.item then
                table.foreachi(self.db.account.buffer.items, function(k, v)
                    self.db.account.buffer.items[k].checked = selected
                end)
            elseif MangAdmin.db.char.requests.favitem then
                table.foreachi(self.db.account.favorites.items, function(k, v)
                    self.db.account.favorites.items[k].checked = selected
                end)
            end
        elseif searchtype == "itemset" then
            if MangAdmin.db.char.requests.itemset then
                table.foreachi(self.db.account.buffer.itemsets, function(k, v)
                    self.db.account.buffer.itemsets[k].checked = selected
                end)
            elseif MangAdmin.db.char.requests.favitemset then
                table.foreachi(self.db.account.favorites.itemsets, function(k, v)
                    self.db.account.favorites.itemsets[k].checked = selected
                end)
            end
        elseif searchtype == "spell" then
            if MangAdmin.db.char.requests.spell then
                table.foreachi(self.db.account.buffer.spells, function(k, v)
                    self.db.account.buffer.spells[k].checked = selected
                end)
            elseif MangAdmin.db.char.requests.favspell then
                table.foreachi(self.db.account.favorites.spells, function(k, v)
                    self.db.account.favorites.spells[k].checked = selected
                end)
            end
        elseif searchtype == "skill" then
            if MangAdmin.db.char.requests.skill then
                table.foreachi(self.db.account.buffer.skills, function(k, v)
                    self.db.account.buffer.skills[k].checked = selected
                end)
            elseif MangAdmin.db.char.requests.favskill then
                table.foreachi(self.db.account.favorites.skills, function(k, v)
                    self.db.account.favorites.skills[k].checked = selected
                end)
            end
        elseif searchtype == "quest" then
            if MangAdmin.db.char.requests.quest then
                table.foreachi(self.db.account.buffer.quests, function(k, v)
                    self.db.account.buffer.quests[k].checked = selected
                end)
            elseif MangAdmin.db.char.requests.favquest then
                table.foreachi(self.db.account.favorites.quests, function(k, v)
                    self.db.account.favorites.quests[k].checked = selected
                end)
            end
        elseif searchtype == "creature" then
            if MangAdmin.db.char.requests.creature then
                table.foreachi(self.db.account.buffer.creatures, function(k, v)
                    self.db.account.buffer.creatures[k].checked = selected
                end)
            elseif MangAdmin.db.char.requests.favcreature then
                table.foreachi(self.db.account.favorites.creatures, function(k, v)
                    self.db.account.favorites.creatures[k].checked = selected
                end)
            end
        elseif searchtype == "object" then
            if MangAdmin.db.char.requests.object then
                table.foreachi(self.db.account.buffer.objects, function(k, v)
                    self.db.account.buffer.objects[k].checked = selected
                end)
            elseif MangAdmin.db.char.requests.favobject then
                table.foreachi(self.db.account.favorites.objects, function(k, v)
                    self.db.account.favorites.objects[k].checked = selected
                end)
            end
        elseif searchtype == "tele" then
            if MangAdmin.db.char.requests.tele then
                table.foreachi(self.db.account.buffer.teles, function(k, v)
                    self.db.account.buffer.teles[k].checked = selected
                end)
            elseif MangAdmin.db.char.requests.favtele then
                table.foreachi(self.db.account.favorites.teles, function(k, v)
                    self.db.account.favorites.teles[k].checked = selected
                end)
            end
        end
        PopupScrollUpdate()
...
function MangAdmin:PrepareScript(object, text, script)
    ...
    if text then
        if self.db.account.style.showtooltips then
            object:SetScript("OnEnter", function()
                GameTooltip:SetOwner(this, "ANCHOR_RIGHT");
                GameTooltip:SetText(text);
                GameTooltip:Show()
            end)
            object:SetScript("OnLeave", function()
                GameTooltip:SetOwner(this, "ANCHOR_RIGHT");
                GameTooltip:Hide()
            end)
        end
...
function MangAdmin:InitButtons()
    -- start tab buttons
    self:PrepareScript(ma_tabbutton_main, Locale["ttGM_tabButton"], function()
        MangAdmin:InstantGroupToggle("main")
    end)
    self:PrepareScript(ma_tabbutton_char, Locale["ttCHAR_CharButton"], function()
        MangAdmin:InstantGroupToggle("char")
    end)
    self:PrepareScript(ma_tabbutton_npc, Locale["tt_NpcButton"], function()
        MangAdmin:InstantGroupToggle("npc");
    end)
    self:PrepareScript(ma_tabbutton_go, Locale["tt_GOButton"], function()
        MangAdmin:InstantGroupToggle("go");
    end)
    self:PrepareScript(ma_tabbutton_tele, Locale["tt_TeleButton"], function()
        MangAdmin:InstantGroupToggle("tele");
    end)
    self:PrepareScript(ma_tabbutton_misc, Locale["tt_MiscButton"], function()
        MangAdmin:InstantGroupToggle("misc")
    end)
    self:PrepareScript(ma_tabbutton_server, Locale["tt_ServerButton"], function()
        MangAdmin:InstantGroupToggle("server")
    end)
    self:PrepareScript(ma_tabbutton_log, Locale["tt_LogButton"], function()
        MangAdmin:InstantGroupToggle("log")
    end)
    self:PrepareScript(ma_tabbutton_who, Locale["tt_whotabmenubutton"], function()
        MangAdmin:InstantGroupToggle("who")
    end)
    -- end tab buttons
    -- start mini buttons
    self:PrepareScript(ma_mm_logoframe, nil, function()
        MangAdmin:OnClick()
    end)
    self:PrepareScript(ma_mm_mainbutton, Locale["ttGM_tabButton"], function()
        MangAdmin:InstantGroupToggle("main")
    end)
    self:PrepareScript(ma_mm_charbutton, Locale["ttCHAR_CharButton"], function()
        MangAdmin:InstantGroupToggle("char")
    end)
    self:PrepareScript(ma_mm_npcbutton, Locale["tt_NpcButton"], function()
        MangAdmin:InstantGroupToggle("npc")
    end)
    self:PrepareScript(ma_mm_gobutton, Locale["tt_GOButton"], function()
        MangAdmin:InstantGroupToggle("go")
    end)
    self:PrepareScript(ma_mm_telebutton, Locale["tt_TeleButton"], function()
        MangAdmin:InstantGroupToggle("tele")
    end)
    self:PrepareScript(ma_mm_ticketbutton, Locale["tt_TicketButton"], function()
        ShowTicketTab()
    end)
    self:PrepareScript(ma_mm_miscbutton, Locale["tt_MiscButton"], function()
        MangAdmin:InstantGroupToggle("misc")
    end)
    self:PrepareScript(ma_mm_serverbutton, Locale["tt_ServerButton"], function()
        MangAdmin:InstantGroupToggle("server")
    end)
    self:PrepareScript(ma_mm_logbutton, Locale["tt_LogButton"], function()
        MangAdmin:InstantGroupToggle("log")
    end)
    self:PrepareScript(ma_mm_whobutton, Locale["tt_whotabmenubutton"], function()
        MangAdmin:InstantGroupToggle("who")
    end)
    -- end mini buttons
    self:PrepareScript(ma_languagebutton, Locale["tt_languageButton"], function()
        MangAdmin:ChangeLanguage(UIDropDownMenu_GetSelectedValue(ma_languagedropdown))
    end)
    self:PrepareScript(ma_itembutton, Locale["tt_ItemButton"], function()
        MangAdmin:TogglePopup("search", {
            type = "item"
        })
    end)
    self:PrepareScript(ma_itemsetbutton, Locale["tt_ItemSetButton"], function()
        MangAdmin:TogglePopup("search", {
            type = "itemset"
        })
    end)
    self:PrepareScript(ma_spellbutton, Locale["tt_SpellButton"], function()
        MangAdmin:TogglePopup("search", {
            type = "spell"
        })
    end)
    self:PrepareScript(ma_skillbutton, Locale["tt_SkillButton"], function()
        MangAdmin:TogglePopup("search", {
            type = "skill"
        })
    end)
    self:PrepareScript(ma_questbutton, Locale["tt_QuestButton"], function()
        MangAdmin:TogglePopup("search", {
            type = "quest"
        })
    end)
    self:PrepareScript(ma_creaturebutton, Locale["tt_CreatureButton"], function()
        MangAdmin:TogglePopup("search", {
            type = "creature"
        })
    end)
    self:PrepareScript(ma_objectbutton, Locale["tt_ObjectButton"], function()
        MangAdmin:TogglePopup("search", {
            type = "object"
        })
    end)
    self:PrepareScript(ma_telesearchbutton, Locale["ma_TeleSearchButton"], function()
        MangAdmin:TogglePopup("search", {
            type = "tele"
        })
    end)
    self:PrepareScript(ma_sendmailbutton, Locale["ma_Mail"], function()
    ...
    self:PrepareScript(ma_searchbutton, nil, function()
        MangAdmin:SearchStart("item", ma_searcheditbox:GetText())
    end)
    self:PrepareScript(ma_resetsearchbutton, nil, function()
        MangAdmin:SearchReset()
    end)
    self:PrepareScript(ma_closebutton, nil, function()
        MangAdmin:CloseButton("bg")
    end)
    self:PrepareScript(ma_popupclosebutton, Locale["tt_closeWindow"], function()
        MangAdmin:CloseButton("popup")
    end)
    self:PrepareScript(ma_popup2closebutton, Locale["tt_closeWindow"], function()
        MangAdmin:CloseButton("popup2")
    end)
...
function MangAdmin:InitDropDowns()
    ...
    local function ReloadTableDropDownInitialize()
        ...
        for k, v in pairs(buttons) do
            ...
            info.checked = nil
            info.func = function()
                UIDropDownMenu_SetSelectedValue(ma_reloadtabledropdown, this.value)
            end
            info.icon = nil
    ...
    local function WeatherDropDownInitialize()
        ...
        for k, v in pairs(buttons) do
            ...
            info.value = v[2]
            info.func = function()
                UIDropDownMenu_SetSelectedValue(ma_weatherdropdown, this.value)
            end
            info.checked = nil
    ...
    local function NpcEmoteDropDownInitialize()
        ...
        for k, v in pairs(buttons) do
            ...
            info.value = v[2]
            info.func = function()
                UIDropDownMenu_SetSelectedValue(ma_npcemotedropdown, this.value)
            end
            info.checked = nil
            -- info.notCheckable = true
            info.icon = nil
    ...
    local function NpcEmoteDropDownInitialize()
        ...
        for k, v in pairs(buttons) do
            ...
            info.value = v[2]
            info.func = function()
                UIDropDownMenu_SetSelectedValue(ma_npcemotedropdown_a, this.value)
            end
            info.checked = nil
            -- info.notCheckable = true
            info.icon = nil
...
function MangAdmin:InitScrollFrames()
    ...
    ma_PopupScrollBar:SetScript("OnVerticalScroll", PopupScrollUpdate(), function(self, offset)
        FauxScrollFrame_OnVerticalScroll(self, offset, 30, PopupScrollUpdate())
    end)
    ma_PopupScrollBar:SetScript("OnShow", function()
        PopupScrollUpdate()
    end)
    -- local zoneupdate = function() Mang:TeleScrollUpdate() end
    ma_ZoneScrollBar:SetScript("OnVerticalScroll", TeleScrollUpdate(), function(self, offset)
        FauxScrollFrame_OnVerticalScroll(self, offset, 16, TeleScrollUpdate())
    end)
    ma_ZoneScrollBar:SetScript("OnShow", function()
        TeleScrollUpdate()
    end)
    ma_SubzoneScrollBar:SetScript("OnVerticalScroll", SubzoneScrollUpdate(), function(self, offset)
        FauxScrollFrame_OnVerticalScroll(self, offset, 16, SubzoneScrollUpdate())
    end)
    ma_SubzoneScrollBar:SetScript("OnShow", function()
        SubzoneScrollUpdate()
    end)
    ...
    ma_whoscrollframe:SetScript("OnVerticalScroll", WhoUpdate(), function(self, offset)
        FauxScrollFrame_OnVerticalScroll(self, offset - 1, 16, WhoUpdate())
    end)
    ma_whoscrollframe:SetScript("OnShow", function()
        WhoUpdate()
    end)
    ...
    ma_maileditbox:SetScript("OnTextChanged", function()
        MangAdmin:UpdateMailBytesLeft()
    end)
    ma_maileditbox:SetScript("OnCursorChanged", function()
    ...
    ma_logframe:SetScript("OnUpdate", function()
        MangAdminLogOnUpdate(self, 0, ma_logframe)
    end)
end
...
function PopupScrollUpdate()
    ...
    if MangAdmin.db.char.requests.item or MangAdmin.db.char.requests.favitem then -- get items
        local count = 0
        if MangAdmin.db.char.requests.item then
            table.foreachi(MangAdmin.db.account.buffer.items, function()
                count = count + 1
            end)
        elseif MangAdmin.db.char.requests.favitem then
            table.foreachi(MangAdmin.db.account.favorites.items, function()
                count = count + 1
            end)
        end
        if count > 0 then
            ...
            for line = 1, 7 do
                ...
                if lineplusoffset <= count then
                    ...
                    -- item icons
                    getglobal("ma_PopupScrollBarEntryIcon" .. line .. "IconTexture"):SetTexture(
                        GetItemIcon(item["itId"]))
                    getglobal("ma_PopupScrollBarEntryIcon" .. line):SetScript("OnEnter", function()
                        GameTooltip:SetOwner(this, "ANCHOR_RIGHT");
                        GameTooltip:SetHyperlink("item:" .. item["itId"] .. ":0:0:0:0:0:0:0");
                        GameTooltip:Show()
                    end)
                    getglobal("ma_PopupScrollBarEntryIcon" .. line):SetScript("OnLeave", function()
                        GameTooltip:SetOwner(this, "ANCHOR_RIGHT");
                        GameTooltip:Hide()
                    end)
                    getglobal("ma_PopupScrollBarEntryIcon" .. line):SetScript("OnClick", function()
                        MangAdmin:AddItem(item["itId"], arg1)
                    end)
                    getglobal("ma_PopupScrollBarEntryIcon" .. line):Show()
                    -- item description
                    getglobal("ma_PopupScrollBarEntry" .. line):SetText(
                        "Id: |cffffffff" .. item["itId"] .. "|r Name: |cffffffff" .. item["itName"] .. "|r")
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnClick", function()
                        MangAdmin:AddItem(item["itId"], arg1)
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnEnter", function()
                        GameTooltip:SetOwner(this, "ANCHOR_RIGHT");
                        GameTooltip:SetHyperlink("item:" .. item["itId"] .. ":0:0:0:0:0:0:0");
                        GameTooltip:Show()
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnLeave", function()
                        GameTooltip:SetOwner(this, "ANCHOR_RIGHT");
                        GameTooltip:Hide()
                    end)
                    ...
                    if MangAdmin.db.char.requests.item then
                        if item["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.items[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.items[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    elseif MangAdmin.db.char.requests.favitem then
                        if item["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.items[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.items[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    end
    ...
    elseif MangAdmin.db.char.requests.itemset or MangAdmin.db.char.requests.favitemset then -- get itemsets
        local count = 0
        if MangAdmin.db.char.requests.itemset then
            table.foreachi(MangAdmin.db.account.buffer.itemsets, function()
                count = count + 1
            end)
        elseif MangAdmin.db.char.requests.favitemset then
            table.foreachi(MangAdmin.db.account.favorites.itemsets, function()
                count = count + 1
            end)
        end
        if count > 0 then
            ...
            for line = 1, 7 do
                ...
                if lineplusoffset <= count then
                    ...
                    local key = lineplusoffset
                    getglobal("ma_PopupScrollBarEntry" .. line):SetText(
                        "Id: |cffffffff" .. itemset["isId"] .. "|r Name: |cffffffff" .. itemset["isName"] .. "|r")
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnClick", function()
                        MangAdmin:AddItemSet(itemset["isId"])
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnEnter", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnLeave", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):Enable()
                    getglobal("ma_PopupScrollBarEntry" .. line):Show()
                    if MangAdmin.db.char.requests.itemset then
                        if itemset["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.itemsets[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.itemsets[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    elseif MangAdmin.db.char.requests.favitemset then
                        if itemset["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.itemsets[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.itemsets[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    end
    ...
    elseif MangAdmin.db.char.requests.quest or MangAdmin.db.char.requests.favquest then -- get quests
        local count = 0
        if MangAdmin.db.char.requests.quest then
            table.foreachi(MangAdmin.db.account.buffer.quests, function()
                count = count + 1
            end)
        elseif MangAdmin.db.char.requests.favquest then
            table.foreachi(MangAdmin.db.account.favorites.quests, function()
                count = count + 1
            end)
        end
        if count > 0 then
            ...
            for line = 1, 7 do
                ...
                if lineplusoffset <= count then
                    ...
                    local key = lineplusoffset
                    getglobal("ma_PopupScrollBarEntry" .. line):SetText(
                        "Id: |cffffffff" .. quest["qsId"] .. "|r Name: |cffffffff" .. quest["qsName"] .. "|r")
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnClick", function()
                        MangAdmin:Quest(quest["qsId"], arg1)
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnEnter", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnLeave", function() --[[Do nothing]]
                    ...
                    if MangAdmin.db.char.requests.quest then
                        if quest["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.quests[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.quests[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    elseif MangAdmin.db.char.requests.favquest then
                        if quest["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.quests[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.quests[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    end
    ...
    elseif MangAdmin.db.char.requests.creature or MangAdmin.db.char.requests.favcreature then -- get creatures
        local count = 0
        if MangAdmin.db.char.requests.creature then
            table.foreachi(MangAdmin.db.account.buffer.creatures, function()
                count = count + 1
            end)
        elseif MangAdmin.db.char.requests.favcreature then
            table.foreachi(MangAdmin.db.account.favorites.creatures, function()
                count = count + 1
            end)
        end
        if count > 0 then
                ...
                if lineplusoffset <= count then
                    ...
                    if MangAdmin.db.char.requests.creature then
                        if creature["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.creatures[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.creatures[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    elseif MangAdmin.db.char.requests.favcreature then
                        if creature["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.creatures[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.creatures[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    end
    ...
    elseif MangAdmin.db.char.requests.spell or MangAdmin.db.char.requests.favspell then -- get spells
        local count = 0
        if MangAdmin.db.char.requests.spell then
            table.foreachi(MangAdmin.db.account.buffer.spells, function()
                count = count + 1
            end)
        elseif MangAdmin.db.char.requests.favspell then
            table.foreachi(MangAdmin.db.account.favorites.spells, function()
                count = count + 1
            end)
        end
        if count > 0 then
            ...
            for line = 1, 7 do
                ...
                if lineplusoffset <= count then
                    ...
                    -- spell info
                    getglobal("ma_PopupScrollBarEntry" .. line):SetText(
                        "Id: |cffffffff" .. spell["spId"] .. "|r Name: |cffffffff" .. spell["spName"] .. "|r")
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnEnter", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnLeave", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnClick", function()
                        LearnAndUnlearnSpell(true, spell["spId"])
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):Enable()
                    getglobal("ma_PopupScrollBarEntry" .. line):Show()
                    if MangAdmin.db.char.requests.spell then
                        if spell["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.spells[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.spells[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    elseif MangAdmin.db.char.requests.favspell then
                        if spell["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.spells[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.spells[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    end
    ...
    elseif MangAdmin.db.char.requests.skill or MangAdmin.db.char.requests.favskill then -- get skills
        local count = 0
        if MangAdmin.db.char.requests.skill then
            table.foreachi(MangAdmin.db.account.buffer.skills, function()
                count = count + 1
            end)
        elseif MangAdmin.db.char.requests.favskill then
            table.foreachi(MangAdmin.db.account.favorites.skills, function()
                count = count + 1
            end)
        end
        if count > 0 then
            ...
            for line = 1, 7 do
                ...
                if lineplusoffset <= count then
                    ...
                    local key = lineplusoffset
                    getglobal("ma_PopupScrollBarEntry" .. line):SetText(
                        "Id: |cffffffff" .. skill["skId"] .. "|r Name: |cffffffff" .. skill["skName"] .. "|r")
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnEnter", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnLeave", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnClick", function()
                        MangAdmin:SetSkill(skill["skId"], nil, nil)
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):Enable()
                    getglobal("ma_PopupScrollBarEntry" .. line):Show()
                    if MangAdmin.db.char.requests.skill then
                        if skill["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.skills[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.skills[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    elseif MangAdmin.db.char.requests.favskill then
                        if skill["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.skills[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.skills[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    end
    ...
    elseif MangAdmin.db.char.requests.object or MangAdmin.db.char.requests.favobject then -- get objects
        local count = 0
        if MangAdmin.db.char.requests.object then
            table.foreachi(MangAdmin.db.account.buffer.objects, function()
                count = count + 1
            end)
        elseif MangAdmin.db.char.requests.favobject then
            table.foreachi(MangAdmin.db.account.favorites.objects, function()
                count = count + 1
            end)
        end
        if count > 0 then
            ...
            for line = 1, 7 do
                ...
                if lineplusoffset <= count then
                    ...
                    local key = lineplusoffset
                    getglobal("ma_PopupScrollBarEntry" .. line):SetText(
                        "Id: |cffffffff" .. object["objId"] .. "|r Name: |cffffffff" .. object["objName"] .. "|r")
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnClick", function()
                        MangAdmin:AddObject(object["objId"], arg1)
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnEnter", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnLeave", function() --[[Do nothing]]
                    ...
                    if MangAdmin.db.char.requests.object then
                        if object["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.objects[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.objects[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    elseif MangAdmin.db.char.requests.favobject then
                        if object["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.objects[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.objects[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    end
    ...
    elseif MangAdmin.db.char.requests.tele or MangAdmin.db.char.requests.favtele then -- get teles
        local count = 0
        if MangAdmin.db.char.requests.tele then
            table.foreachi(MangAdmin.db.account.buffer.teles, function()
                count = count + 1
            end)
        elseif MangAdmin.db.char.requests.favtele then
            table.foreachi(MangAdmin.db.account.favorites.teles, function()
                count = count + 1
            end)
        end
        if count > 0 then
            ...
            for line = 1, 7 do
                ...
                if lineplusoffset <= count then
                    ...
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnClick", function()
                        MangAdmin:ChatMsg(".tele " .. tele["tName"])
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnEnter", function() --[[Do nothing]]
                    end)
                    getglobal("ma_PopupScrollBarEntry" .. line):SetScript("OnLeave", function() --[[Do nothing]]
                    ...
                    if MangAdmin.db.char.requests.tele then
                        if tele["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.teles[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.buffer.teles[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    elseif MangAdmin.db.char.requests.favtele then
                        if tele["checked"] then
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.teles[key]["checked"] = false;
                                PopupScrollUpdate()
                            end)
                        else
                            getglobal("ma_PopupScrollBarEntry" .. line .. "ChkBtn"):SetScript("OnClick", function()
                                MangAdmin.db.account.favorites.teles[key]["checked"] = true;
                                PopupScrollUpdate()
                            end)
                        end
                    end
...
function MangAdmin:CloseButton(name)
    ...
    elseif name == "popup2" then
        FrameLib:HandleGroup("popup2", function(frame)
            frame:Hide()
        end)
    end
end
