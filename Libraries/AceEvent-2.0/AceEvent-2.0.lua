...
if not AceLibrary:HasInstance("AceOO-2.0") then
    error(MAJOR_VERSION .. " requires AceOO-2.0")
end

local AceOO = AceLibrary:GetInstance("AceOO-2.0")
local Mixin = AceOO.Mixin
local AceEvent = Mixin {"RegisterEvent", "RegisterAllEvents", "UnregisterEvent", "UnregisterAllEvents", "TriggerEvent",
                        "ScheduleEvent", "ScheduleRepeatingEvent", "CancelScheduledEvent", "CancelAllScheduledEvents",
                        "IsEventRegistered", "IsEventScheduled", "RegisterBucketEvent", "UnregisterBucketEvent",
                        "UnregisterAllBucketEvents", "IsBucketEventRegistered", "ScheduleLeaveCombatAction",
...
function AceEvent:TriggerEvent(event, ...)
    AceEvent:argCheck(event, 2, "string")
    local AceEvent_registry = AceEvent.registry
    if (not AceEvent_registry[event] or not next(AceEvent_registry[event])) and
        (not AceEvent_registry[ALL_EVENTS] or not next(AceEvent_registry[ALL_EVENTS])) then
        return
    end
        ...
        while obj do
            ...
            if type(method) == "string" then
                ...
                if obj_method then
                    ...
                    if not success then
                        geterrorhandler()(
                            err:find("%.lua:%d+:") and err or (debugstack():match("(.-: )in.-\n") or "") .. err)
                    end
                end
            elseif method then -- function
                local success, err = pcall(method, ...)
                if not success then
                    geterrorhandler()(err:find("%.lua:%d+:") and err or (debugstack():match("(.-: )in.-\n") or "") ..
                                          err)
                end
            end
            tmp[obj] = nil
            ...
            if not cont then
                ...
                if t == "string" then
                    ...
                    if obj_method then
                        local success, err = pcall(obj_method, obj, ...)
                        if not success then
                            geterrorhandler()(err:find("%.lua:%d+:") and err or
                                                  (debugstack():match("(.-: )in.-\n") or "") .. err)
                        end
                    end
                elseif t == "function" then -- function
                    ...
                    AceEvent:error("Cannot trigger event %q. %q's handler, %q, is not a method or function (%s).",
                        event, obj, method, t)
                end
            end
            tmp[obj] = nil
    ...
    if AceEvent_registry[ALL_EVENTS] then
        ...
        while obj do
            ...
            elseif t == "function" then
                ...
                AceEvent:error("Cannot trigger event %q. %q's handler, %q, is not a method or function (%s).", event,
                    obj, method, t)
            end
            tmp[obj] = nil
            obj = next(tmp)
...
function AceEvent:IsEventRegistered(event)
    ...
    local AceEvent_registry = AceEvent.registry
    if self == AceEvent then
        return AceEvent_registry[event] and next(AceEvent_registry[event]) or AceEvent_registry[ALL_EVENTS] and
                   next(AceEvent_registry[ALL_EVENTS]) and true or false
    end
    if AceEvent_registry[event] and AceEvent_registry[event][self] then
...
local function activate(self, oldLib, oldDeactivate)
    ...
    self.postInit = oldLib and oldLib.postInit or self.playerLogin and ChatTypeInfo and ChatTypeInfo.WHISPER and
                        ChatTypeInfo.WHISPER.r and true
    ...
    SlashCmdList["IN"] = function(msg)
        ...
        self:ScheduleEvent("AceEventSlashIn-" .. math.random(1, 1000000000), editbox.Execute, seconds, editbox,
            command .. rest)
    end
    ...
    self:RegisterEvent("LOOT_OPENED", function()
        if GetRealNumRaidMembers() > 0 or GetRealNumPartyMembers() > 0 then
